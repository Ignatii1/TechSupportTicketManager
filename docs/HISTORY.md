...
- Ревью (`/code-review high`), 10 находок, исправлены все: по неполному списку (ошибка, потолок страниц) пропуски не
  ...
  импорт пишет settings.json раз, а не на каждую заявку; `_myId` не запоминается от сменённого клиента; справочник
  статусов живёт час; F5 и автообновление перечитывают общим `RecheckAsync`; уведомление режется до 63/250 символов
  (H.NotifyIcon бросал на длинном) и не роняет таймер; settings.json пишется через временный файл.
- Проверено: сборка, SelfCheck (правило пропусков). **Не проверено:** таймер, уведомления и щелчок по ним на Windows.
