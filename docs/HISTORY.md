...
- Не `ChangedMoreThan`, а тот же список «мои открытые», что у импорта: он уже проверен на живом сервере, а у
  ...
  Карточки, выпавшие из списка (закрыты, переданы, чужие), перечитываются по одной, до 20 за раз, давние первыми.
...
- Закрытые сам не переносим (решение 0.4.0: без согласия — никогда): уведомление, по щелчку — вопрос F5
  (`AskMoveClosed`, теперь общий). О каждой закрытой — один раз за запуск. Итог — в заголовке доски, ошибки — в лог
  по одной, а не каждые 5 минут.
