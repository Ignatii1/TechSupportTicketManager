...
- Проверено: сборка, SelfCheck (правило пропусков, чтение и запись settings.json). **Не проверено:** таймер,
  уведомления, щелчок по ним и счётчик в заголовке на Windows.
