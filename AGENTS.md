...
| Creator / executors in the panel | Parsed in `.Parse.cs` (`Field`, `Names`), stored on `Ticket` (`Creator`, `Executors`, `ExecutorGroup`), rows 5–6 of the panel grid in `MainWindow.xaml` (`SelectableBody`). Cards from before 0.8.0 are filled on first open (`OnSelectedTicketChanged`, once per run). Also in the relay's `TB ticket` answer (`ClaudeRelay.People`). |
